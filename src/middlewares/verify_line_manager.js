...
*/
	static async checkIfLineManagerExist(req, res, next) {
		const foundManager = await UserService.findUserByAttribute({ id: req.body.managerId });
		if (foundManager.role !== 'lineManager') {
			ResponseService.setError(403, 'The user has to be a lineManager to be assigned a user');
