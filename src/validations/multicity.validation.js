...
export const validateTrip = async (req, res, next) => {
	const schema = Joi.object({
		...
		reason: Joi.string().min(5).trim().regex(/^[a-zA-Z .]+$/)
			.required()
			.messages({
				'string.base': 'travelling reason must be a string',
				'string.empty': 'Reason must not be empty',
				'string.pattern.base': 'Please know the difference between travel reason and telephone number',
				'any.required': 'travel reason is required'
			}),
		departureFrom: Joi.number().integer().min(1)
			.required()
			.messages({
		...
		departureTo: Joi.number().integer().min(1)
			.required()
			.messages({
				'number.base': 'city must be a an id',
				'number.empty': 'departure from city must not be empty',
	...
	const schema = Joi.array()
		.items(Joi.object({
			...
			returningDate: Joi.date().greater(req.body.startingDate).utc().format('YYYY-MM-DD')
				.required()
				.messages({
					'date.greater': 'date of returning from travel must be greater than departure date',
					'date.format': 'date of travel must have format YYYY-MM-DD',
					'any.required': 'travel date is required'
				}),
			name: Joi.string().min(5).trim().regex(/^[a-zA-Z .]+$/)
				.required()
				.messages({
